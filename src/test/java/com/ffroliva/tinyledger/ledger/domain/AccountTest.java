...
class AccountTest {
    ...
    private List<LedgerEvent> historyWith(long minorUnits) {
        ...
        if (minorUnits > 0) {
            history.addAll(Account.rehydrate(history)
                    .deposit(new Deposit("alice", id, UUID.randomUUID(), new Money(GBP, minorUnits), null), T));
        }
        return history;
    }
    ...
    void rehydrateAppliesWithdrawalAndKeepsOpeningDetails() {
        ...
        history.addAll(account.withdraw(
                new Withdraw("alice", account.id(), UUID.randomUUID(), new Money(GBP, 2_000), null), T));
    ...
    void currencyMismatchIsRejectedAsStateNotShape() {
        ...
        List<LedgerEvent> events =
                account.deposit(new Deposit("alice", account.id(), UUID.randomUUID(), Money.of("EUR", 100), null), T);
    ...
    void withdrawalInAnotherCurrencyIsRejectedToo() {
        Account account = openedWith(5_000);
        List<LedgerEvent> events =
                account.withdraw(new Withdraw("alice", account.id(), UUID.randomUUID(), Money.of("EUR", 100), null), T);
        MovementRejected rejected = (MovementRejected) events.getFirst();
