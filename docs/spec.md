...
| 9 | **E2E** | `docker compose up`, then the **seven** unmocked scenarios driven by `ledger-cli` against a running `full`-profile application (§9.6) — since #11 that application is **the container image**, brought up as a Compose service, so what CI exercises is what would be deployed. The host-jar path is retained behind `E2E_MODE=jar`, but **no CI job runs it** — this job builds the image, not the jar — so it was verified by hand once (7 passed) and will drift unobserved until a matrix leg covers it. Said plainly, because the branch was kept precisely to avoid silent coverage loss and as wired it *is* that loss. The pytest-bdd binding over the *whole* catalogue, and the README `curl` extraction (§8.3), remain planned | every push |
...
| 11 | **Security** | `gitleaks`, then the image is **built and Trivy-scanned** (`CRITICAL,HIGH`, `exit-code: 1`, `ignore-unfixed: true`) — both in the `security` job **because that job is a required check**: a scan in a non-required job is as unable to stop a merge as one that exits 0. `detect-secrets` stays unwired: same class of finding as `gitleaks`, so running both is redundant scanning rather than extra safety. Trivy needs no credential, so a fork still gets the image scan | every push |
| 11b | **Dependency-Check** | **OWASP Dependency-Check** over the build tree at `failBuildOnCVSS=7`, in its **own `depcheck` job**. It is *intended* to see **test-scope** dependencies, which never reach the image — the only real gap it closes over Trivy, and a small one. **That is currently UNPROVEN:** the plugin defaults `skipTestScope` to `true` (now set explicitly to `false`) and its `check` goal declares `requiresDependencyResolution` of `compile+runtime`, so Maven may not resolve test artifacts for it at all. Until a run is observed reporting a test-scope jar, this job's whole justification is a hypothesis, and it is recorded as one. **It is separate because it is slow**: inside `security` it did not complete in **80 minutes** on a cold NVD cache (run `31229974027`), while the rest of that job takes about four. A required check that slow is one people route around. The cold cost is an **unfixed ceiling**, named in `ci.yml`: a cache written on a branch is invisible to `main`, so the first run after a merge pays it again; the upgrade path is `nvdDatafeedUrl`. **Skipped, not passed**, without `NVD_API_KEY`. **Not currently in main's required checks** — adding one is a branch-protection change the repository owner must make, so today it fails the workflow but cannot block a merge | every push |
