...
| **Reached at** | `https://…:8443`; the plaintext entrypoint answers **301** and serves nothing |
| **Floor** | TLS 1.2 |
| **Edge headers** | HSTS (one year), `X-Content-Type-Options`, `X-Frame-Options` |
| **Application ports** | **not published at all** — `8080` and `9090` are in-network only |

**TLS stops at Traefik, deliberately.** The proxy-to-application hop and every backing-service hop
(Postgres, Redis, Kafka) stay plaintext, as does Keycloak. That is a **named gap**, not an
oversight: a service mesh is the tool for it, and ADR 0005 records why one is out of scope. **Do
not read "the ledger uses TLS" as "the ledger encrypts everything".**

**Keycloak is not fronted by Traefik**, and that is a decision rather than an omission. It derives
`iss` from the `Host` header, so routing it through a TLS terminator changes the issuer string for
the CLI, `application-full.properties`, `ci.yml` and `ledger-cli/config.py` at once — to buy TLS on
a fixture identity provider marked *never deploy*. In a deployment the IdP is a managed service with
its own certificate. **No issuer or audience validation was relaxed anywhere in this work**; §6.4's
...
The file provider is used instead: one router, one service, written by hand. Adding TLS is not a
