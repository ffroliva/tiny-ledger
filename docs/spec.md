...
The starting point was a deliberately small brief written for this project: three features in a few
hours, in-memory storage, no auth, no monitoring, no atomic operations. This specification goes far
beyond that on purpose, so the repository ships **two run modes from one codebase**:

| Mode | Command | What runs | Purpose |
|---|---|---|---|
| **`standalone`** (default) | `./mvnw spring-boot:run` | In-memory event store, in-memory cache, no auth, no broker. Binds `127.0.0.1` only; the startup banner prints `AUTH DISABLED (standalone)`. **JDK 25** is the only prerequisite. | The minimal runtime in one command: clone, run, curl the APIs. The scope beyond the minimal brief is a recorded, deliberate choice (`agentic-workflow.md` §6). |
...
configuration does. The minimal run and the full production stack execute *the same
...
**rename**: the issuer moved to `https://auth.localhost/realms/tiny-ledger` everywhere at once — **nine operational files** as of v3.48, listed in `docs/docker.md` alongside the command that regenerates the list. It said *eight* until then, and had undercounted since the string spread past the five files it named,
...
| Asset class or symbol mismatch | 422 | `/errors/asset-mismatch` |
| Insufficient asset holding | 422 | `/errors/insufficient-holding` |
| Concurrent modification | 409 | `/errors/version-conflict` |
| Reused movement UID, different payload | 409 | `/errors/idempotency-conflict` |
| Caller already holds `ledger.accounts.max-per-owner` accounts | 409 | `/errors/account-limit-reached` |
...
| `PUT` | `/api/v1/accounts/{accountUid}/asset-transfers/{transferUid}` | Transfer ETF assets with tax-lot allocation (IN acquisition / OUT disposal with HIFO/FIFO). |
...
papers, routers and registries (`docs/adr/`, `INDEX.md`) are exempt — ADRs and
...
| 3 | **Architecture** | `ApplicationModules.verify()` + ArchUnit (§9.2), **and `WorkflowGovernanceTest` over `.github/workflows/` itself** — see "The pipeline is fenced too" below | every push |
...
| 11e | **ZAP API scan** | **`zap-api-scan.py -S` over `docs/api/openapi.yaml`**, in the same `zap` job, after the baseline. It **enumerates the nine operations from the contract** instead of crawling for links, which is what lifts 11c's ceiling: that ceiling was structural, not a configuration miss, and no amount of authentication would have moved it — a spider with no hyperlinks has nothing to follow. Measured on the same stack in the same run: **14 URLs against the baseline's 3, and 119 passive rules exercised against its 66**. **`-S` is load-bearing:** `zap-api-scan.py` runs an **active** scan by default, and this repository's rule is a baseline and never an active scan — no attack payload against a ledger. Safe mode skips it. **WHAT IT ACTUALLY REACHES — corrected at v3.50, and the correction is this row's own overstatement.** It said the scan *"writes to the ledger with the contract's own example values"*. It does not, and the artifact says so: ZAP's OpenAPI import substitutes **no path parameters** — `AccountUid` is `type: string, format: uuid` with no `example:`, and no importer can chain a POST response into a later path parameter — so requests go out verbatim as `/api/v1/accounts/{accountUid}/balance`. **Half of that reason was wrong, and v3.52 measured it:** `AccountUid` now carries an `example:` and the path templates still go out verbatim (`%7BaccountUid%7D`, run `31322575966`). ZAP's OpenAPI import substitutes **no path parameter, example or not** — the missing example was never the cause, so the seed step below has to hand ZAP a rewritten copy of the contract and cannot just fill this field in. Measured on run `31278543144`: alert `100000` ("A Client Error response code was returned by the server") **x8**, so **eight of the nine operations 4xx**. Nothing is written, and no balance is ever read. **The first half stopped being true at v3.52**: query parameters and request bodies *are* built from the contract's examples, so `openAccount` now sends `{"name":"ACC-001","currency":"GBP"}`, answers `201` and **creates an account on every scan**. It is the one operation that left the client-error list — measured **7**, not 8, on run `31322575966`. The ceiling stays at 8 until a seed step lands, so the drop is recorded here rather than gated. Two of the nine would `403` regardless, because the token is minted for `alice` (`ledger:writer`, `ledger:reader`) while `/audit/entries` and `/accounts/*/events` require `ledger:auditor`, which belongs to `dave`. **What survives is header coverage, and it is real**: passive header rules do not care that a body is an error, so all nine responses are evaluated — proven when rule 10035 fired **x9** the moment HSTS was genuinely removed. What this stage does *not* do is exercise the API's data paths. **That gap is now gated rather than described:** ci.yml pins the client-error count at a ceiling of **8**, because ZAP records those at INFORMATIONAL risk and `zap_common.py` skips Informational when it sets the exit code, so they could never fail the job through the scanner. Fix the substitution and that number drops, with the run that proves it. **Two gates, both differential, because a clean report is the failure mode here** — an inert token means every route answers 401, the passive rules read nine error bodies, and the scan reports clean, which is indistinguishable from a clean API: (1) the token is proven live **before** ZAP is handed it, the same URL requested twice, requiring `200` with the bearer and `401` without — non-empty is not proof, since an expired or wrong-audience token 401s just as silently; (2) the scan must report **more URLs than the baseline's 3**, a control measured on the same run rather than assumed. **Its first finding is the argument for its existence:** `Cross-Origin-Resource-Policy` missing (rule 90004), which fires on a response a browser could embed — invisible to the baseline **by construction**, since an error body has nothing to protect. Fixed on the terminator rather than dispositioned, per `.zap/rules.tsv`'s own rule. **The gate's proof is two real runs**, one variable apart: `31277939324` without the header reported `WARN-NEW: 1` and exited non-zero; `31278543144` with it reported `WARN-NEW: 0` and `PASS` 118 → **119** — the rule moved to PASS rather than disappearing, which is what separates *fixed* from *no longer evaluated*. Dispositions live in `.zap/api-rules.tsv`, separate from `rules.tsv` because that file dispositions the **edge** and this one the **routes**. **Not a required check**, for the same branch-protection reason as 11b and 11c | every push |
...
**The pipeline is fenced too.** Every stage above gates something in `src/`; until v3.56 nothing
gated `ci.yml`, and it had drifted exactly as an unfenced surface does.
`WorkflowGovernanceTest` (stage 3, alongside ArchUnit) now holds every workflow to four rules, each
recording what it was fixing:

| Rule | The state it found |
|---|---|
| Top-level `permissions:` present and **read-only** | `ci.yml` had **no block at all** — eleven jobs, two of them holding `SONAR_TOKEN` and `NVD_API_KEY`, all on the repository's default `GITHUB_TOKEN` scopes. `depcheck.yml` has had one since it was written. Read-only is the right value here because no job writes to the repository: the image is built and scanned, never published (stage 12); ZAP sets `allow_issue_writing: false`; Sonar decorates through its own GitHub App |
| Every job declares `timeout-minutes` ≤ 120 | **One** occurrence across eleven jobs. The other ten carried GitHub's 360-minute default, so a hung step meant a six-hour bill on a *required* check nobody could clear. Now set per job from **measured** durations on run `32582281310` (gate 0m, unit 1m, integration 3m, security 2m, cli 0m, e2e 3m, load 0m, sonar 1m, iac 1m, k8s-kind 3m), bounded well above rather than at the measurement. 120 is the ceiling because `depcheck`'s cold NVD run is 70 minutes |
| Third-party actions pinned to a 40-character commit SHA | Five floated on mutable tags — `trivy-action`, `setup-uv` (twice), `action-baseline`, `setup-terraform`, `kind-action`. A tag is repointable by its owner between two runs, and these run against this repository's token. `actions/*` is exempt: GitHub-owned, so pinning it buys nothing a tag does not. Dependabot's existing `github-actions` group bumps the pins weekly |
| `continue-on-error: true` only in allowlisted jobs | Allowed only in `load`, where a Gatling threshold breach *is* the run whose report and log someone needs, and a hard failure would skip the uploads carrying them. Everywhere else it is a gate that cannot fail — the same defect the deleted stage 6, the pre-v3.41 Sonar step and the un-`exit-code`d Trivy step each shipped |

**It is differential by construction (`AGENTS.md` trap 8), and that is the half that makes it a check
rather than an observation.** A checker asserted only against a tree that already passes scores the
same whether its parser works or is inert. So the same code path runs over synthetic workflows that
**must** score hits — a missing block, a top-level `write`, a floating ref, an unallowlisted
`continue-on-error` — and one that **must** score zero. The green run over `.github/workflows/` is
only half the evidence; `depcheck.yml` scoring zero on the real tree is the other real-world half.

**Its own first bug is the argument for that design.** The timeout pattern anchored end-of-line
immediately after the digits, so `timeout-minutes: 20   # measured 1m` — valid YAML — did not match,
and it reported seven jobs as untimed while the values sat in the file. A checker only ever run
against a passing tree would have reported that as a real finding.

It parses lines rather than YAML deliberately: no YAML parser is on this project's test classpath,
and adding a dependency to lint four structural shapes is not a trade worth making. The shapes are
column-anchored — jobs at two spaces, job keys at four — and jobs are read only from under the
column-0 `jobs:` key, because scanning the whole file for that shape also matches `push:`,
`pull_request:` and `workflow_dispatch:` under `on:`, three phantom jobs each reported as missing a
...
Tracked here and in §15's assumptions. The design-review passes ran
...
| 3.48 | 2026-08-08 | **The audit's first findings were produced by the audit's own predecessor commits, which is the most useful thing it could have shown.** Stage 11e (the ZAP API scan) landed in #30 and left **three** stale claims behind it, all introduced by the change that closed the gap they describe. (1) **§12.1 had no row for stage 11e at all** — 11d got one in the same session and 11e did not, so the stage table described a pipeline missing one of its jobs. Added. (2) **Row 11c said the upgrade `zap-api-scan.py` "is not wired"**, which stopped being true in the commit that wrote 11e; the row went on asserting it. (3) **`.zap/rules.tsv` said the same thing twice** — *"It is not wired, and would need a token minted for it"* — in the file whose whole job is to record what this gate cannot see, which is the first place a reader checks whether a limit still applies. Both corrected in place. **A measured value was also restated rather than carried:** 11c's ceiling was recorded as **two URLs**, measured against `https://127.0.0.1:8443`; at the current target (443) the same scan reaches **`Total of 3 URLs`**. The old number was not wrong when written and was wrong when read, which is the failure mode this pass exists for. **One correction runs the other way and is worth recording because it caught the auditor:** a dead citation to `docs/security.md` (a file that has never existed here) was found independently by two passes, and the first replacement asserted that `docs/security-material.md` does **not** carry the HSTS gap. It does, at its edge-headers row. A false claim written while fixing a false claim, and it survived until the second pass collided with it — evidence for the rule that a correction is checked, never assumed. **`docs/security-material.md` and `docs/urls-and-tls.md` both enumerated the edge headers and both predated `Cross-Origin-Resource-Policy`**, which 11e's first finding added; both updated. **The commits ported here were authored as `Claude <noreply@anthropic.com>`**, which `AGENTS.md` forbids — the author is the person accountable for the change, and a machine co-author on a public history is noise in `git log`, `git blame` and `git shortlog`. Re-authored on the port rather than merged as they were **A SYSTEMATIC SWEEP OF THE MEASURABLE CLAIMS FOUND ONE MORE, AND WHAT IT TEACHES IS THE SHAPE OF THE FIX RATHER THAN THE FACT.** Ports, pinned tool versions and counts were checked mechanically against the things they describe. Ports are clean — every surviving `8081` reference is explicitly past-tense, which is correct, since Keycloak stopped publishing it at v3.44. Tool pins are clean, and for a structural reason worth keeping: the documents restate exactly one of them (`zaproxy/action-baseline@v0.15.0`), so there is almost nothing to drift. Counts are clean where they are derived from something a reader can run — **nine** CI jobs, **four** default-profile containers, **nine** OpenAPI operations, **27** Cucumber scenarios, and the e2e suite's **seven** scenarios (exactly seven `def test_` under a module-level `pytest.mark.e2e` in `ledger-cli/tests/test_e2e_scenarios.py`) all verified true. **The one that had rotted was the one nobody could check:** three documents said the issuer string "appears in **eight places**", enumerated as five files plus three documents. It is **nine operational files** plus six documents — it undercounted on both halves, because the string spread into `dynamic.yml`, `gen-dev-ca.sh`, `restart-replay.sh` and `LedgerSimulation.java` after the sentence was written. Nothing was wrong when written; it rotted in place, which is precisely this pass's subject. **Corrected by replacing the number with the list AND the one-line command that regenerates it**, rather than with a newer number that would rot the same way — a count a reader can re-derive cannot go stale silently, and a bare one always does. The v3.44 revision row keeps its *eight*: it records what that change did at the time, and rewriting history to match the present is a different error. |
| 3.49 | 2026-08-09 | **HSTS was being SENT while five documents said it was not, and the reason it survived is that the claim was asserted against the wrong component.** `docker/traefik/dynamic.yml`, `.zap/rules.tsv`, `docs/pitfalls.md`, `docs/urls-and-tls.md` and `docs/security-material.md` all recorded `Strict-Transport-Security` as deliberately not sent, with a correct and specific reason — the `app` router is a catch-all, so a browser dialling `https://localhost` receives a pin on the **bare host**, and HSTS is host-scoped and PORT-INDEPENDENT, so a one-year pin there force-upgrades `http://localhost:3000` and every other local development server on the machine, clearable only through `chrome://net-internals/#hsts`. **Every one of those five described the TERMINATOR, where the header genuinely was never configured. The APPLICATION sent it.** `SecurityConfig` carried no `headers` configuration at all, so Spring Security's default `HstsHeaderWriter` applied, and `application.properties`' `server.forward-headers-strategy=native` makes `RemoteIpValve` mark proxied requests secure — that writer's only condition. So the documented hazard was shipping, to anyone following the README. **The scanner reported it on every run and nobody read it:** ZAP rule 10035 showed `PASS` in both stage 11c and stage 11e, and for that rule PASS means the header is **present** — while `.zap/rules.tsv` dispositioned 10035 as `IGNORE` on the premise that it was absent, pre-silencing the one alarm that could have said so. **Fixed in code rather than in prose**, because the documented reasoning was sound and only the implementation was missing: `SecurityConfig#hstsOff()` on all three chains, gated by `SecurityConfigTest#hstsIsNotSentOnASecureRequest`. **`.secure(true)` is the load-bearing part of that test** — MockMvc requests are insecure by default, the writer only runs on a secure request, and the identical assertion without it passes with the fix deleted, which is trap 8 exactly. Red proof measured: *"Response should not contain header 'Strict-Transport-Security'"*. A second assertion is the control, requiring `X-Content-Type-Options` to survive so the first cannot pass by the whole headers chain being off. **A deployment-level differential confirms it beyond the unit test:** the same ZAP job reported `PASS: Strict-Transport-Security [10035]` before the fix and `WARN-NEW … x 9` after it, on nine routes, through the proxy — which is also stage 11e's first firing on a real change to what it watches. **Two collateral findings.** (1) Sonar failed `new_security_rating` on two `java:S4502` CSRF findings whose lines had not changed: adding one line to each chain expression promoted pre-existing code into the **new-code** gate. The rating moved because the diff moved, not because the posture did; `managementChain` had carried that suppression since it was written and the other two chains had the identical argument and no annotation. (2) The four `application*.properties` files are now **ASCII-only** — 133 characters converted, all in comments, key counts unchanged. Not a corruption fix; the files were valid UTF-8. `java.util.Properties#load(InputStream)` is *specified* as ISO-8859-1 and IntelliJ's properties-file encoding setting overrides `.editorconfig` outright, so the same bytes rendered as `â€"`, `Â§` and `â”€` for one reader and correctly for another. `PropertiesAreAsciiTest` guards it, with a control that fails if its file glob ever matches nothing |
| 3.50 | 2026-08-09 | **Stage 11e's own row, and three other documents, described a scan that had never happened — and the artifact proving it had been published by the same job all along.** The claim was that importing the contract *"sends one request per operation and three of those are writes, so the scan writes to the ledger with the contract's own example values"*, reading *"nine routes with live 200 JSON responses, account balances among them"*. **All of it was written from what the contract DESCRIBES rather than from what the scan DID.** The `zap-api-scan` artifact of run `31278543144` shows the path templates going out verbatim — `/api/v1/accounts/{accountUid}/balance?consistency=strong`, `/api/v1/accounts/{accountUid}/deposits/{depositUid}` — because ZAP's OpenAPI import substitutes no path parameters: `AccountUid` is `type: string, format: uuid` with no `example:`, and no importer can chain a POST response into a later path parameter. Alert `100000` ("A Client Error response code was returned by the server") carries **8 instances**: eight of the nine operations 4xx. **Nothing was written and no balance was ever read.** Two of the nine would `403` regardless, since the token is `alice`'s (`ledger:writer`, `ledger:reader`) while the auditor routes need `ledger:auditor`, held by `dave` — so the liveness probe at the front of that step, being a read, proves the token authenticates and proves nothing about the roles the scan's other operations need. **The coverage that remains is real and is the reason the stage stays:** passive header rules do not care that a body is an error, so all nine responses are evaluated, which is exactly how v3.49's HSTS defect was caught on nine routes at once. **What was missing was a check that could tell the difference**, and that is the substantive change here rather than the wording: `Total of N URLs` counts what ZAP IMPORTED, not what answered, and read 14 on the very run where eight operations were failing — the trap-8 blind spot sitting inside the step written to close it. ZAP records client errors at INFORMATIONAL risk and `zap_common.py` skips Informational when it sets the exit code, so they could never fail the job through the scanner. `ci.yml` now reads them out of the JSON report and gates on a **ceiling of 8**, proven differentially against the published artifact before landing: ceiling 8 passes at the measured 8, ceiling 7 fails, a missing report fails (`-1`, not `0`), an alert-free report passes at `0`, and truncated JSON fails. **The lesson is narrower than "check your claims":** every false sentence here was checkable against a file this job had already uploaded. The evidence was not missing, it was unread — the same failure as v3.49's, where the scanner had reported the HSTS header on every run |
